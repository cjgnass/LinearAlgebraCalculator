...
function isValidVector(expr: Expression): boolean {
...
function simplifyAddition(
    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < cols; col++) {
            ...
            const outputRightElement = simplify(
                rightMatrixExpr.matrix[row][col],
            );
...
function simplifyDot(expr: BinaryExpression): NumberLiteral {
    const left = simplify(expr.left);
    const right = simplify(expr.right);
    if (!isValidVector(left) || !isValidVector(right)) return nanExpr;
    const leftTranspose = getMatrixTranspose(left as MatrixExpression);
    const outputExpr = multiplyMatrices(
        leftTranspose,
        right as MatrixExpression,
    );
    if (outputExpr.kind !== "MatrixExpression") return nanExpr;
    const outputMatrix = outputExpr.matrix;
    const output = outputMatrix[0][0];
    if (output.kind !== "NumberLiteral") return nanExpr;
...
function simplifyCross(
    expr: BinaryExpression,
): MatrixExpression | NumberLiteral {
    const left = simplify(expr.left);
    const right = simplify(expr.right);
    if (!isValidVector(left) || !isValidVector(right)) return nanExpr;
    const leftDimensions = getMatrixDimensions(left as MatrixExpression);
    const rightDimensions = getMatrixDimensions(right as MatrixExpression);
    const leftRows = leftDimensions[0];
    const leftCols = leftDimensions[1];
    const rightRows = rightDimensions[0];
    const rightCols = rightDimensions[1];
    if (leftRows !== 3 || rightRows !== 3) return nanExpr;
    if (leftCols !== 1 || rightCols !== 1) return nanExpr;
    const leftMatrix = (left as MatrixExpression).matrix;
    const iLeft = simplify(leftMatrix[0][0]) as NumberLiteral;
    const jLeft = simplify(leftMatrix[1][0]) as NumberLiteral;
    const kLeft = simplify(leftMatrix[2][0]) as NumberLiteral;
    const rightMatrix = (right as MatrixExpression).matrix;
    const iRight = simplify(rightMatrix[0][0]) as NumberLiteral;
    const jRight = simplify(rightMatrix[1][0]) as NumberLiteral;
    const kRight = simplify(rightMatrix[2][0]) as NumberLiteral;
    const outputMatrix: Expression[][] = [[], [], []];
    outputMatrix[0].push({
        kind: "NumberLiteral",
        value: jLeft.value * kRight.value - kLeft.value * jRight.value,
        start: 0,
        end: 0,
    });
    outputMatrix[1].push({
        kind: "NumberLiteral",
        value: kLeft.value * iRight.value - iLeft.value * kRight.value,
        start: 0,
        end: 0,
    });
    outputMatrix[2].push({
        kind: "NumberLiteral",
        value: iLeft.value * jRight.value - jLeft.value * iRight.value,
        start: 0,
        end: 0,
    });
...
export function simplify(expr: Expression): NumberLiteral | MatrixExpression {
    switch (expr.kind) {
        case "BinaryExpression":
            ...
            if (expr.op === ".") return simplifyDot(expr);
            if (expr.op === "x") return simplifyCross(expr);
            break;
        case "NumberLiteral":
